...
export function SalesCardBack({
  ...
  return (
    ...
    >
      ...
      {scoutReport ? (
        <div className="bg-[#F5F5EE] px-4 py-2.5 flex-shrink-0">
      ...
      <div className="relative bg-[#0F0F0F] text-white flex-shrink-0 mt-auto">
...
function Meta({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      <div className="text-[8px] font-black tracking-widest text-gray-400 uppercase mb-0.5">{label}</div>
      <div className="font-semibold text-gray-900 truncate text-[11px]">{value}</div>
    </div>
  );
}
...
function Th({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <th className={`px-1.5 py-1.5 font-black tracking-wider uppercase text-[8px] text-right whitespace-nowrap ${className}`}>
      {children}
    </th>
  );
}
