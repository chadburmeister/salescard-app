...
export interface SalesCardFrontProps {
  /** Display name (e.g. "Morgan Lee") */
  name: string;
  /** Role title (e.g. "Senior AE") */
  role: string;
  /** 1-100 SalesCard Score */
  score: number;
  /** Username slug (used in linkedin URL display) */
  linkedinHandle?: string;
  /** Optional photo URL */
  photoUrl?: string;
  /** 4 sub-grades (0-10 scale) */
  subGrades?: {
    PIPELINE: number;
    WIN_RATE: number;
    QUOTA: number;
  ...
  /** Cert / card number to display (e.g. "00249201") */
  certNo?: string;
...
/**
 * SalesCard front-of-card.
 * Ported from the landing-page SVG. Server-renderable.
 */
export function SalesCardFront({
  ...
  const handle = linkedinHandle ?? slug(name);
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={`SalesCard for ${name}, ${tier.label} ${score}`}
    >
      {/* slab */}
      ...
      {photoUrl ? (
        <>
          <defs>
            <clipPath id={`photoClip-${handle}`}>
              <rect x="30" y="220" width="400" height="290" />
            </clipPath>
          </defs>
          <image href={photoUrl} x="30" y="220" width="400" height="290" preserveAspectRatio="xMidYMid slice" clipPath={`url(#photoClip-${handle})`} />
        </>
      ) : (
        <>
      ...
      <text x="60" y="538" fontFamily="Inter, system-ui, sans-serif" fontSize="14" fontWeight="600" fill="#1F2937" fontStyle="italic">
